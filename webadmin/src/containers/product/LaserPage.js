...
export default class LaserPage extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <ContentHeader title="Laser" small={title} >
        {
          item.isView === 'VIEW' ?
  ...
  newSaveData (data) {
    ...
    if (this.state.isEdit === true) {
      ...
      const newData = {data: temp, id: data.id, typ: 'laser'};
      this.updateData(newData);
    } else {
      const temp = {
        id: data.id, mode: data.mode, typ: 'laser', product_name: data.product_name,  
