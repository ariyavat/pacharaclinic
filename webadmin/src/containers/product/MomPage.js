...
export default class MomPage extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <ContentHeader title="คุณแม่หลังคลอด" small={title} >
        {
          item.isView === 'VIEW' ?
  ...
  newSaveData (data) {
    ...
    if (this.state.isEdit === true) {
      ...
      const newData = {data: temp, id: data.id, typ: 'mom'};
      this.updateData(newData);
    } else {
      const temp = {
        id: data.id, mode: data.mode,  typ: 'mom', product_name: data.product_name,  unit: data.unit, price: data.price, 
        title: data.title, con: data.con, send: data.send, detail: data.detail,
        status: data.status, img: data.img, state: data.state,
      };        
      this.saveData(temp);
