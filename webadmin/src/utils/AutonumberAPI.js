...
export function getAutonumber(id,cn) {
  return axios.get(`${config.init.ms_url}autonumber/${id}/${cn}`);
}

export function createAutonumber(data) {
  return axios.post(`${config.init.ms_url}createAutonumber`, JSON.stringify(data));
}
...
export function updateAutonumber(data) {
  ...
  return axios.post(`${config.init.ms_url}updateAutonumber`, JSON.stringify(data));
}

export function setAutonumber(id, lnum) {
    const temp = {lastID: lnum};
    const cn = JSON.parse(localStorage.getItem('msClinic')).location_id; 
    const newData = {data: temp, id: id, cn: cn};
    return axios.post(`${config.init.ms_url}updateAutonumber`, JSON.stringify(newData));   
}
