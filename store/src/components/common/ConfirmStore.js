...
export default class ConfirmStore extends Component {
  ...
  componentDidMount(){
    ...
    if(this.props.mode === 'PO'){
      this.setState({ title: 'ยืนยันการสั่งซื้อวัตถุดิบ !', text: 'บันทึกใบสั่งซื้อเรียบร้อย'  })
    }
    if(this.props.mode === 'TRN'){
      this.setState({ title: 'ยืนยันการโอนสินค้า !', text: 'บันทึกใบโอนสินค้าเรียบร้อย'  })
    }
