...
import {getProductSet} from '../../utils/ProductAPI';
import {
  getTempBAdjusts, getBReciveProduct, createTempBAdjust, updateTempBAdjust, deleteTempBAdjust, deleteTempBAdjusts, 
  createBAdjust, createBAdjustList, updateBStore
} from '../../utils/StoreBAPI';
...
export default class BAdjForm extends Component {
  ...
  handleAddItem(e){
    ...
    if (item.pid !== '') {
      if (item.qty !== '' || item.qty > 0) {
      	...
      	getBReciveProduct(item.pid)
      	.then(function(results) { 
      		if (results.data.status === '200') { 

		      	if(item.typ_id === 'P'){
			        getProductSet(item.pid)
			        .then(function(results) {          
			          if (results.data.status === '200') {  
      ...
      .then(function(results) { 
        if (results.data.status === '201') {
          ...
          tempList.forEach(function(val) {
            ...
            createBAdjustList(newData);   
            updateBStore(newData, 'A', _this.state.mem, data.dat); 
          });
          let delData = {cn: ''} ;
          deleteTempBAdjusts(delData);         
