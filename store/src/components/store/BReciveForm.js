...
import {getTempBRecives, createTempBRecive, updateTempBRecive, deleteTempBRecive, deleteTempBRecives, createBRecive, createBReciveList, updateBStore} from '../../utils/StoreBAPI';
...
export default class BReciveForm extends Component {
      				...
      				<div className="col-sm-4 col-sm-offset-1">
      					<div className="row">
      		            	...
      		            	<div className="col-xs-12 col-sm-12 form-group">
      		            		<label>อ้างอิง</label>
		                        <select className="form-control" id="rno"  ref="rno" value={item.rno}> 
		                            <option value="-"></option>                        
		                            {item.rList.map(createRno)}     
		                        </select> 
      		            	</div>
      					</div>
      				</div>			
  ...
  getTempRecives () {
    let _this = this;
    const users = JSON.parse(localStorage.getItem('msClinic'));
    getTempBRecives(users.location_id)
    .then(function(results) {
      if (results.data.status === '200') {
      ...
      .then(function(results) {      
        if (results.data.status === '201') {
          ...
          tempList.forEach(function(val) {
            ...
            createBReciveList(newData);           
            updateBStore(newData, 'I', _this.state.mem, data.dat);
          });
          let delData = {cn: '-'} ;
          deleteTempBRecives(delData); 
