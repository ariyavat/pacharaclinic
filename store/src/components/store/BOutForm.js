...
import {getTempBOuts, createTempBOut, updateTempBOut, deleteTempBOut, deleteTempBOuts, createBOut, createBOutList, updateBStore} from '../../utils/StoreBAPI';
...
export default class BOutForm extends Component {
      ...
      .then(function(results) { 
        if (results.data.status === '201') {
          ...
          tempList.forEach(function(val) {
            ...
            createBOutList(newData);          
            updateBStore(newData, 'P', _this.state.mem, data.dat); 
          });
          let delData = {cn:  '- '} ;
          deleteTempBOuts(delData);         
