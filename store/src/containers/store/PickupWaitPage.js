...
import {getPickups, getPickupDetail, updatePickup} from '../../utils/StoreAPI';


const initialState = {
  loading: false, isViews: 'LIST', tempList: [],  productList: [], data: null
};
...
export default class PickupWaitPage extends React.Component {
  ...
  handleSaveEditResult(ino){
    ...
    .then(function(results) { 
      if (results.data.status === '200') {
        ...
        _this.setState({ dList: results.data.data });
      }
    });
  ...
  render() {
    ...
    <div>
      {
        item.isViews !== 'PRINT' ?
        <ContentHeader title="Back Office" small="รายการเบิกสินค้า   ssss" >
        {
          item.isViews === 'VIEWS' ?
      ...
      {
      	item.isViews === 'FORM' && item.data !== null ?
      	<PickupWaitForm 
      	    mode='WAIT'
      	    data={item.data}  
      		  onSave={this.handleSaveResult.bind(this)}
            onSaveEdit={this.handleSaveEditResult.bind(this)}
