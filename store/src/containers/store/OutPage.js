...
import OutForm from '../../components/store/OutForm';
import ReciveA4 from '../../components/store/ReciveA4';

import {getOuts, getOutDetail} from '../../utils/StoreAPI';
import {getProducts} from '../../utils/ProductAPI';

const initialState = {
  loading: false, isViews: 'LIST', tempList: [],  productList: [], data: []
};
const curent_date = moment().format('YYYY-MM-DD');
...
export default class OutPage extends React.Component {
  ...
  render() {
    ...
    <div>
      {
        item.isViews !== 'PRINT' ?
        <ContentHeader title="คลังวัตถุดิบ" small="จ่ายออกจากคลัง" >
        {
          item.isViews === 'VIEWS' ?
